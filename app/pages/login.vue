  ...
  <div class="auth-page">
    <section class="auth-shell">
      <div class="auth-copy">
        ...
        <h1 class="heading-accent">{{ $t("auth.login.title") }}</h1>
        <p>
      ...
      <form class="auth-form" @submit.prevent="submit">
        ...
        <button class="btn btn-primary auth-submit" type="submit" :disabled="userStore.loading">
          {{ userStore.loading ? $t("auth.login.submitting") : $t("auth.login.submit") }}
        </button>

        <p v-if="message" class="auth-message" :class="{ error: hasError }" role="status">
          {{ message }}
        </p>

        <p class="switch-copy">
          {{ $t("auth.login.switchPrompt") }}
          <NuxtLink :to="registerLink">{{ $t("auth.login.switchLink") }}</NuxtLink>
        </p>
      </form>
    </section>
  </div>
...
async function submit() {
  ...
  try {
    ...
    message.value = error.includes("credencials")
      ? t("auth.login.errors.invalid")
      : error === "login_failed"
        ? t("auth.login.errors.default")
      : error || t("auth.login.errors.default");
  }
}
